...
whether a market is tradable right now (spreads, activity, sanity checks).
...
def run_risk_agent(ctx: Dict[str, Any], thresholds: Dict[str, Any] = DEFAULT_THRESHOLDS) -> Dict[str, Any]:
    ...
    score = min(s_spread, s_act, s_sanity)
    ...
    key_reasons = [r for r in reasons if "failed" in r.lower() or "very wide" in r.lower() or "zero" in r.lower()]
    if not key_reasons:
